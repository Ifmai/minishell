...
/*   Updated: 2023/02/24 18:23:16 by hozdemir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    execute_command(char *true_path, char **command, int i)
{
    ...
    if(data->pid[i] == 0)
    {
        if(i == 0)
            dup2(data->fd[1], 1);
        else if(i > 0 && i < data->dvd_str->pipe_count - 1)
            dup2(data->fd[0],0);
        //else if(i == data->dvd_str->pipe_count - 1)
            //Close fd;
        execute(true_path, command, data->env);
        exit(0);
    }
    else
